...
class LoginController extends AbstractController {
    ...
    public function loginAction(Request $request): Response {
        ...
        try {
            $jsonResponse = json_decode($responseBody, true, 512, JSON_THROW_ON_ERROR);
        } catch (\JsonException $e) {
            $this->logger->error('JSON decode error: ' . $e->getMessage() . ', Body: ' . $responseBody);
            $this->lastError = 'Authentication service error. Please try again later.';
            $this->lastErrorDetails = 'Invalid response from authentication service';
            return $this->login();
        }
